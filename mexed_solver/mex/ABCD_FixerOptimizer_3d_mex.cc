...
void ABCD_FixerOptimizer_3d_mex_mexFunction(int num_of_lhs,
	...
	for (int pi = 0; pi < 2; pi++) {
		...
		solverSpec[pi].invalid_penalty.resize(3, 0); 
		...
		//FieldDataToArray<bool>(struct_pnt[pi], "verbose", &solverSpec[pi].verbose, 1);


		//For testing LGB version
	...
	std::vector<Eigen::Vector4i> triangle(tri_num);
	...
	for (size_t i = 0; i < tri_num; i++) {
		...
		triangle[i][3] = mesh[i + 3 * tri_num] - 1;
	}
	...
	distortion_kernel::DistortionKernel3D* kernel[2] = { NULL,NULL};
	...
	for (int pi = 0; pi < 2; pi++) {
		...
		case 0:
			...
			kernel[pi] = new distortion_kernel::ARAPKernel3D();
			break;
		case 2:
			std::cout << "\nmeasure#" << pi << " Modified Symmetric Dirichlet energy";
			kernel[pi] = new distortion_kernel::SymDirichletFilteredKernel3D();
			kernel[pi]->EnableFlipFilter(true);
			break;
		case 3:
			kernel[pi] = new distortion_kernel::FlipPenaltyKernel3D(solverSpec[pi].invalid_penalty);
	...
	std::vector<Eigen::Vector3d> buffer = deformed;
	...
	//initializing block relatated data
	int mesh_struct_rhs_index = (num_of_rhs >= 10)? 10+2 : 6+2; 
	...
	std::map<int,bool>	thread_calls; //for reporting statistics
	...
	while (total_iter < iter_num && !is_converged) {
		for (int pi = 0; pi < 2;pi++) {
			...
			for (int cycle_iter = 0; (cycle_iter < solverSpec[pi].cycle_num) && (total_iter < iter_num) ; cycle_iter++) {
				...
				//switching between fixer and optimizer  
				...
				if (solverSpec[pi].is_global) {
					...
					solverSpec[pi].non_empty_block_num = 1;//global mode
					...
					search_dirSpec.non_empty_block_num = 1;
					...
					if (is_inversion_free_stage_global && !model.invalid_element_num) {
						...
						for (int j = 0; j < 2; j++) {
							solverSpec[j].is_global = true; 
							solverSpec[j].non_empty_block_num = 1;
					...
					if (! solverSpec[pi].is_global) //block partitioning part
					{
						...
						double K = 1e16; //unused 
						if (solverSpec[pi].K_hat_size > 0) { //using input thesholds for LGB 
				...
				double BCD_iteration_end = clock();
				double current_timing = double(BCD_iteration_end - ABCD_begin) / CLOCKS_PER_SEC;
				...
				std::cout << "\n-----Iteration " << total_iter << ": " << ((solverSpec[pi].solver_num) ? "PN" : "GD")
					...
					<< ", time= " << current_timing <<
					   ", energy_diff= " << model.energy_difference << 
					std::endl;
			
				invalids_per_iter.push_back(model.invalid_element_num);
				energy_num_per_iter.push_back(solverSpec[pi].energy_num);
				time_per_iter.push_back(current_timing);

				model.return_search_dir = false;
				model.invalid_element_num = 0;

				clock_t ABCD_optimize_begin = clock();
				...
				clock_t ABCD_optimize_end = clock();

				#ifdef _OUTPUT_HESSIAN 
